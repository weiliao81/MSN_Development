...
library("gamlss")
library("ggplot2")
x <- read.csv('withinDispersion.csv')
centile <- data.frame(
  age = seq.int(8, 89,  length.out = 1000)
)
within_gamlss_t <-array(0,dim=c(1,7))
within_gamlss_p <-array(0,dim=c(1,7))
...
################################################################################ step 1: Gamlss fitting
  withinDispersion <- x[,i+3]
  
  m0 <- gamlss(withinDispersion ~ cs(age, df = 5)+sex+eTIV, 
               sigma.fo = ~cs(age, df = 3)+sex+eTIV,
               family = GG,
               method = mixed(30, 100),
               data = x,
               trace = FALSE)
  fn <- function(p) AIC(gamlss(withinDispersion ~ cs(age, df = p[1])+sex+eTIV, 
  ...
  command_gamlss <-paste0("model <- gamlss(withinDispersion ~ cs(age, df = opAIC$par[1])+sex+eTIV, 
  ...
  within_gamlss_t[i] = resInfo[2,3]
  within_gamlss_p[i] = resInfo[2,4]
